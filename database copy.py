...
def registrar_impresion(nombre_pieza, tiempo, lista_materiales, id_impresora, user_id):
    """
    lista_materiales: Es una lista de tuplas [(id_bobina, gramos_usados), ...]
    """
    conn = get_connection()
    cursor = conn.cursor()
    try:
        ...
        # 1. Procesar cada material usado
        for id_bobina, peso in lista_materiales:
            # Obtener datos de la bobina
            cursor.execute("SELECT peso_actual, costo FROM bobinas WHERE id = ?", (id_bobina,))
            datos_bobina = cursor.fetchone()
            
            if datos_bobina:
                peso_actual, costo_rollo = datos_bobina
                
                # Validar stock (Opcional, por ahora dejamos que baje a negativo si es necesario)
                # if peso_actual < peso: return False, f"Falta filamento en bobina ID {id_bobina}"

                # Calcular costo de ESTE material
                precio_por_gramo = costo_rollo / 1000
                costo_este_tramo = precio_por_gramo * peso
                
                costo_total_material += costo_este_tramo
                peso_total += peso

                # DESCONTAR STOCK
                cursor.execute("UPDATE bobinas SET peso_actual = peso_actual - ? WHERE id = ?", (peso, id_bobina))

        # 2. Calcular Costo Energético
        cursor.execute("SELECT power_kw FROM impresoras WHERE id = ?", (id_impresora,))
        res_imp = cursor.fetchone()
        kw_maquina = res_imp[0] if res_imp else 0
        
        # Importar costo zona desde config (truco para no pasarla por param)
        from config import COSTO_KW_ZONA
        costo_energia = kw_maquina * tiempo * COSTO_KW_ZONA
        
        costo_final = costo_total_material + costo_energia

        # 3. Guardar en Historial
        # Usamos la PRIMERA bobina de la lista como referencia para la tabla (id_bobina)
        id_bobina_ref = lista_materiales[0][0]
        
        cursor.execute('''
            INSERT INTO impresiones (
                nombre_pieza, costo_final, peso_usado, tiempo_usado, 
                id_impresora, id_bobina, user_id, fecha, ganancia
            ) VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'), 0)
        ''', (nombre_pieza, costo_final, peso_total, tiempo, id_impresora, id_bobina_ref, user_id))

        # 4. Sumar horas a la impresora
        cursor.execute("UPDATE impresoras SET horas_uso = horas_uso + ? WHERE id = ?", (tiempo, id_impresora))

        conn.commit()
        return True, f"Registrado!\nMaterial: ${costo_total_material:.0f} | Luz: ${costo_energia:.0f}"
...
def obtener_balance_ultimos_dias(user_id, dias=7):
    """
    Devuelve una lista de tuplas con (fecha, total_costo, total_ingreso)
    para los últimos 'dias' días que tengan actividad.
    """
    conn = get_connection()
    cursor = conn.cursor()
    
    # Usamos strftime para agrupar por día (YYYY-MM-DD)
    # COALESCE convierte nulos en 0 para que no falle la suma
    query = '''
        SELECT 
            strftime('%Y-%m-%d', fecha) as dia, 
            SUM(costo_final) as costos, 
            SUM(COALESCE(precio_unit, 0) * COALESCE(cantidad, 1)) as ingresos
        FROM impresiones 
        WHERE user_id = ? 
        GROUP BY dia 
        ORDER BY dia DESC 
        LIMIT ?
    '''
    cursor.execute(query, (user_id, dias))
    data = cursor.fetchall()
    conn.close()
    
    # La consulta devuelve de más nuevo a más viejo, lo invertimos para el gráfico
    return data[::-1]

def obtener_todos_usuarios():
    """Trae todos los usuarios para la pantalla de selección de perfiles"""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT username, nombre_emprendimiento FROM usuarios")
    datos = cursor.fetchall()
    conn.close()
    return datos